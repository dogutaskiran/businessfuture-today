...
function ArticleBody({markdown,inlineImages=[]}:{markdown?:string;inlineImages?:readonly InlineImage[]}){
  ...
  const insertion=new Map<number,InlineImage>();
  inlineImages.forEach((image,i)=>{if(!paragraphIndices.length)return;const ratio=i===0?.38:.70;const target=paragraphIndices[Math.min(paragraphIndices.length-1,Math.max(0,Math.floor(paragraphIndices.length*ratio)))];insertion.set(target,image);});
  return <>{blocks.map((block,index)=>{
    ...
    const image=insertion.get(index);return <div className="article__block" key={index}>{node}{image?<InlineFigure image={image}/>:null}</div>;
  })}</>;
}

export default async function StoryPage({params}:Props){const{slug}=await params;const story=stories.find((item)=>item.slug===slug);if(!story)notFound();return <main><header className="site-header shell"><Link href="/" className="brand"><span>BUSINESS FUTURE</span><strong>TODAY</strong></Link><Link className="back-link" href="/">← Back to today</Link></header><article className="article shell"><p className="eyebrow">{story.kicker}</p><h1>{story.title}</h1><p className="article__dek">{story.dek}</p><div className="story-meta"><span>{story.category}</span><span>{story.readTime}</span></div>{story.heroImage?<figure className="article__hero"><img src={story.heroImage} alt={story.imageAlt||story.title}/>{story.imageCredit?<figcaption>{story.imageCredit}</figcaption>:null}</figure>:null}<div className="article__body"><ArticleBody markdown={story.bodyMarkdown} inlineImages={story.inlineImages}/></div>{story.sourceUrls?.length?<div className="article__sources"><h2>Sources</h2><ul>{story.sourceUrls.map((url)=><li key={url}><a href={url} target="_blank" rel="noreferrer">{new URL(url).hostname}</a></li>)}</ul></div>:null}</article><section className="briefing briefing--article shell"><div><p className="eyebrow">STAY AHEAD</p><h2>The future of business, in your inbox.</h2></div><div><SubscribeForm/><small>Built to become personal, not noisy.</small></div></section></main>;}

...
export function PublicationHeader({ template, pathname = "/" }: { template: PublicationTemplate; pathname?: string }) {
  return <>
    ...
    <header className="pub-header pub-shell">
      <Link href={`/?template=${template.id}`} className="pub-brand" aria-label={`${publicationBrand.name} home`}><img src="/brand/site-header-wordmark-600x130.png" alt="Business Future Today" /></Link>
...
export function PublicationFooter({ template }: { template: PublicationTemplate }) {
  return <footer className="pub-footer">
    <div className="pub-shell pub-footer__grid">
      <div><Link href={`/?template=${template.id}`} className="pub-brand pub-brand--footer"><img src="/brand/site-header-wordmark-600x130.png" alt="Business Future Today" /></Link><p>{publicationBrand.tagline}</p></div>

...
async function cooldown(code:string,msg:string,min=CREDIT_WAIT){await db().query(`INSERT INTO automation_provider_state(provider,state,cooldown_until,last_error_code,last_error) VALUES($1,'cooldown',NOW()+($2::text||' minutes')::interval,$3,$4) ON CONFLICT(provider) DO UPDATE SET state='cooldown',cooldown_until=EXCLUDED.cooldown_until,last_error_code=EXCLUDED.last_error_code,last_error=EXCLUDED.last_error,updated_at=NOW()`,[PROVIDER,String(min),code,msg.slice(0,2000)])}
...
async function defer(id:string,n:number,e:GenError,min:number,dead=false){await db().query(`UPDATE automation_generation_queue SET status=$2,attempts=$3,next_attempt_at=NOW()+($4::text||' minutes')::interval,last_attempt_at=NOW(),locked_at=NULL,last_error_code=$5,last_error=$6,updated_at=NOW() WHERE cluster_id=$1`,[id,dead?"dead":"retry_wait",n,String(min),e.code,e.message.slice(0,4000)])}

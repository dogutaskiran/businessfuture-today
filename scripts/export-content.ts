...
type Row = {
  ...
  hero_path:string|null; card_path:string|null; og_path:string|null; alt_text:string|null; attribution_text:string|null; source_image_url:string|null; license_status:string|null;
...
async function main(){
  const result=await db().query<Row>(`
    SELECT d.slug,d.kicker,d.title,d.dek,d.category,d.body_markdown,d.source_urls,d.published_at,
      h.hero_path,h.card_path,h.og_path,h.alt_text,h.attribution_text,h.source_image_url,h.license_status,
  ...
  const stories=result.rows.map((row,index)=>({
    ...
    heroImage:row.hero_path,cardImage:row.card_path,ogImage:row.og_path,imageAlt:row.alt_text,imageCredit:row.attribution_text,sourceImageCandidate:row.source_image_url,licenseStatus:row.license_status,
    inlineImages:[
